...
# Classe local, que será herdada pelos locais do mapa, constrói a imagem do local na posição do mapa e na dimensão desejada, também verifica
# se há a colisão do player com esse local
class Local(pygame.sprite.Sprite):
    ...
    def is_collision(self, player):
        if (player.player_pos.x + player.TAM_WIDTH > self.rect.left and
                player.player_pos.x - player.TAM_WIDTH < self.rect.right and
    ...
    def handle_collision(self, player):
        if player.player_pos.x < self.rect.left:
            ...
            player.player_pos.x = self.rect.right + player.TAM_WIDTH
        elif player.player_pos.y < self.rect.top:
